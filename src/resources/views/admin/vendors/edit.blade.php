...
@section('content')
    <div class="mb-6">
        ...
        <h1 class="mt-2 text-2xl font-semibold text-slate-900">業者編集</h1>
    </div>

    <form method="post" action="{{ route('admin.vendors.update', $vendor) }}" class="max-w-3xl space-y-6 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        @csrf
        @method('put')
        @include('admin.vendors._form', ['vendor' => $vendor, 'categories' => $categories])
