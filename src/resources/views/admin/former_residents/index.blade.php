...
@section('content')
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <h1 class="text-2xl font-semibold text-slate-900">退去者一覧</h1>
    </div>

    <form method="get" action="{{ route('admin.former-residents.index') }}" class="mb-6 flex flex-wrap items-end gap-4 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
        <div>
        ...
        @if (!empty($filters['property_id']))
            <a href="{{ route('admin.former-residents.index') }}" class="text-sm text-slate-600 underline hover:text-slate-900">クリア</a>
        @endif
    </form>
    ...
    <div class="overflow-x-auto rounded-lg border border-slate-200 bg-white shadow-sm">
        <table class="min-w-full divide-y divide-slate-200 text-sm">
            ...
            <tr>
                ...
                <th class="px-4 py-3 text-left font-medium text-slate-700">最終更新</th>
            ...
            @forelse ($residents as $r)
                <tr>
                    ...
                    <td class="px-4 py-3 text-slate-600">{{ $r->updated_at?->timezone('Asia/Tokyo')->format('Y-m-d H:i') ?? '—' }}</td>
                    <td class="px-4 py-3 text-right">
                        @php
                            $showParams = ['resident' => $r];
                            if (! empty($filters['property_id'])) {
                                $showParams['return_property_id'] = $filters['property_id'];
                            }
                        @endphp
                        <a href="{{ route('admin.former-residents.show', $showParams) }}" class="text-slate-700 underline hover:text-slate-900">詳細</a>
                    </td>
                </tr>
            @empty
                <tr>
                    <td colspan="6" class="px-4 py-8 text-center text-slate-500">退去者がいません。</td>
                </tr>
            @endforelse
            </tbody>
        </table>
